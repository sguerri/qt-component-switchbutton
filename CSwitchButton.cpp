...
CSwitchAnimator::CSwitchAnimator(QObject* target, QObject* parent) : QVariantAnimation(parent) {
    setTargetObject(target);
}

CSwitchAnimator::~CSwitchAnimator() {
    stop();
}

QObject* CSwitchAnimator::targetObject() const {
    return target.data();
}

void CSwitchAnimator::setTargetObject(QObject* _target) {
...
void CSwitchAnimator::updateCurrentValue(const QVariant& value) {
    Q_UNUSED(value);
...
void CSwitchAnimator::updateState(QAbstractAnimation::State newState, QAbstractAnimation::State oldState) {
...
void CSwitchAnimator::setup(int duration, QEasingCurve easing) {
    setDuration(duration);
    setEasingCurve(easing);
}

void CSwitchAnimator::interpolate(const QVariant& _start, const QVariant& end) {
    setStartValue(_start);
    setEndValue(end);
    start();
}

void CSwitchAnimator::setCurrentValue(const QVariant& value) {
    setStartValue(value);
    setEndValue(value);
...
CSwitchSelectionControl::CSwitchSelectionControl(QWidget* parent) : QAbstractButton(parent) {
    setObjectName("SelectionControl");
    setCheckable(true);
}

CSwitchSelectionControl::~CSwitchSelectionControl() {
...
*/
Qt::CheckState CSwitchSelectionControl::checkState() const {
    return isChecked() ? Qt::Checked : Qt::Unchecked;
}

void CSwitchSelectionControl::checkStateSet() {
    const auto state = checkState();
    emit stateChanged(state);
    toggle(state);
}

void CSwitchSelectionControl::nextCheckState() {
    QAbstractButton::nextCheckState();
    CSwitchSelectionControl::checkStateSet();
}



void CSwitchButton::init() {
    setFont(style.font);
    setObjectName("Switch");
    /* setup animations */
    thumbBrushAnimation = new CSwitchAnimator{ this, this };
    trackBrushAnimation = new CSwitchAnimator{ this, this };
    thumbPosAniamtion = new CSwitchAnimator{ this, this };
...
QRect CSwitchButton::indicatorRect() {
...
CSwitchButton::CSwitchButton(const QString& text, QWidget* parent) : CSwitchButton(parent) {
    setText(text);
}

CSwitchButton::CSwitchButton(const QString& text, const QBrush& brush, QWidget* parent) : CSwitchButton(text, parent) {
...
void CSwitchButton::resizeEvent(QResizeEvent* e) {
    shadowPixmap = CSwitchStyle::drawShadowEllipse(THUMB_RADIUS, SHADOW_ELEVATION, QColor(0, 0, 0, 70));
    CSwitchSelectionControl::resizeEvent(e);
}

void CSwitchButton::toggle(Qt::CheckState state) {
    if (state == Qt::Checked) {
